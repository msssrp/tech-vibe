...
const CertificateSec: React.FC<certificateProps> = ({ certificateId }) => {
  ...
  const {
    ...
    ownerOfToken,
    error,
  } = UseCertificateData(certificateId, provider, setProvider);
  ...
  if (error === "Invalid certificate ID" && isLoading)
    return (
      <div className="flex flex-col space-y-4 items-center justify-center mt-5">
        <div className="btn btn-sm">
          Invalid Certificate ID: The specified certificate ID does not exist.
        </div>
        <Link href="/certificate" className="btn btn-sm">
          Checkout all certificates
        </Link>
      </div>
    );
  if (certData && !error)
        ...
        <div className="flex flex-col w-1/2 space-y-4">
          <div className="w-full flex flex-col items-start justify-start">
            ...
            <div className="p-3 flex items-center space-x-3 border w-full shadow-md">
              <button
                onClick={() =>
                  clipboard.copy(`${gateway_url}/ipfs/${certificateData.image}`)
                }
              >
                {clipboard.copied ? (
                  <svg
