...
const ModalName: React.FC<modalNameProps> = ({ userId, userName }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      if (!newDisplayName) return setErrorText("Please provide a new name.");
      const error = await updateFullname(newDisplayName, userId);
      if (error.error) return setErrorText(error.error);
      close();
      notifications.show({
        title: "update status",
        message: `updated ${newDisplayName} to new name!!`,
        onClose: () => {
