...
import { updateUserSocialTwitter } from "@/libs/actions/user/user_social";
import { notifications } from "@mantine/notifications";
import { useRouter } from "next/navigation";

type modalTwitterProps = {
  userId: string;
  twitterLink: string;
};

const ModalTwitter: React.FC<modalTwitterProps> = ({ userId, twitterLink }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (!newTwitterLink)
        return setErrorText("Please provide a new twitter link.");
      const error = await updateUserSocialTwitter(newTwitterLink, userId);
      if (error.error) return setErrorText(error.error);
      close();
      notifications.show({
        title: "update status",
        message: `updated ${newTwitterLink} to new twitter link!!`,
        onClose: () => {
        ...
        <form className="px-7 space-y-5 py-3" onSubmit={handleSubmit}>
          ...
          <label className="rounded-none border-b flex items-center">
            <input
              ...
              onChange={(e) => {
                setNewTwitterLink(e.target.value);
                setErrorText("");
              }}
            />
          </label>
