...
import { updateUserSocialFacebook } from "@/libs/actions/user/user_social";
...
const ModalFacebook: React.FC<modalFacebookProps> = ({
  userId,
  facebookLink,
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (!newFacebookLink)
        return setErrorText("Please provide a new facebook link.");
      const error = await updateUserSocialFacebook(newFacebookLink, userId);
      if (error.error) return setErrorText(error.error);

      close();
      notifications.show({
        title: "update status",
        message: `updated ${newFacebookLink} to new facebook link!!`,
        onClose: () => {
        ...
        <form className="px-7 space-y-5 py-3" onSubmit={handleSubmit}>
          ...
          <label className="rounded-none border-b flex items-center">
            <input
              ...
              onChange={(e) => {
                setNewFacebookLink(e.target.value);
                setErrorText("");
              }}
            />
          </label>
