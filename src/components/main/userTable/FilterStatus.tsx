import { UserProviders } from "@/libs/actions/user/userClient";
    ...
    onClick={() =>
      setColumFilters((prev) => {
        const existingFilter = prev.find(
          (filter) => filter.id === "user_provider"
        );

        if (existingFilter?.value === status) {
          // If the status is already selected, remove the filter
          return prev.filter((filter) => filter.id !== "user_provider");
        } else {
          // Otherwise, add the selected status filter
          return prev
            .filter((filter) => filter.id !== "user_provider")
            .concat({ id: "user_provider", value: status });
        }
      })
    }
  >
      ...
      <Popover.Dropdown>
        <Text size="xs">
          <div className="flex flex-col items-start justify-center space-y-2">
            ...
            <p className=" text-slate-800 text-base">provider</p>
            {UserProviders.map((status) => (
              <StatusBtn
