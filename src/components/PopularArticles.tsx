  ...
  return (
    <div className="my-8 sm:my-12">
      <div className="md:ml-5 lg:ml-10">
        <h2 className="text-3xl sm:text-4xl text-center sm:text-left">Popular articles</h2>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mt-5 lg:mt-12 px-5 sm:px-10 lg:px-60">
        {populararticleslist.map((populararticles) => {
          return (
            <div className="flex justify-center cursor-pointer">
              <div className="card card-compact w-80 sm:w-[30rem] h-96 bg-base-100 shadow-xl rounded-t-[50px]">
                <figure>
                  <img
                ...
                <div className="card-body ">
                  <span className="space-x-1 line-clamp-1">
                    {tags.map((badge) => (
                      <div key={badge.id} className={`badge ${badge.color} text-white py-3`}>
                        {badge.text}
                      </div>
                    ))}
                  </span>
                  <h2 className="card-title">{populararticles.title}</h2>
                  <p className="line-clamp-2">{populararticles.description}</p>
